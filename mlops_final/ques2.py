...
def run_splits(data, label, train_frac, dev_frac, parameters):
...
def run_multiple_models(data, label, parameters, no_of_splits, train_frac, dev_frac, test_frac,):
    assert train_frac + dev_frac + test_frac == 1.
    results = {}
    for split in range(no_of_splits):
        data, label = random_split(data, label)
        res = run_splits(data, label, train_frac, dev_frac, parameters)
        results[split] = res
    return results
