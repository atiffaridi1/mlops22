...
def run_splits(data, label, train_frac, dev_frac, parameters, args):
    ...
    for model_name in parameters:
        ...
        if not save_model_flag:
            path = 'models/{}_{}.pkl'.format(args.clf_name, args.random_state)
            # save
            with open(path,'wb') as f:
                pickle.dump(best_model,f)
                save_model_flag = True
            path2 = 'results/{}_{}.txt'.format(args.clf_name, args.random_state)
            with open(path2, 'w') as ff:
                print("best_clf_rep['macro avg']['f1-score'])", best_clf_rep['macro avg']['f1-score'])
                txt = 'test accuracy: {}\ntest macro-f1: {}'.format(best_metric, best_clf_rep['macro avg']['f1-score'])
                ff.write(txt)
                f.close()
    return results

def run_multiple_models(data, label, parameters, no_of_splits, train_frac, dev_frac, test_frac, args):
    assert train_frac + dev_frac + test_frac == 1.
    results = {}
    for split in range(no_of_splits):
        data, label = random_split(data, label)
        res = run_splits(data, label, train_frac, dev_frac, parameters, args)
        results[split] = res
    return results
...
def main(args):
    if args.clf_name=='svm':
        parameters = {'svc': {'model': SVC, 'params': {'kernel': ['rbf'], 'gamma': [0.01], 'C': [0.1]}}}
    elif args.clf_name=='tree':
        parameters = {'decision_tree': {'model': DecisionTreeClassifier, 'params': {'criterion': ['entropy'], 'max_features': ['auto']}}}
    else:
        assert False
    ...
    results = run_multiple_models(data, label, parameters, no_of_splits, train_frac, dev_frac, test_frac, args)
    # print(results)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--clf_name', default='svm', type=str)
